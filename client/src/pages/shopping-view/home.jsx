...
import { BabyIcon, Cat, Check, ChevronLeftIcon, ChevronRightIcon, CloudLightning, Heading, RabbitIcon, ShirtIcon, TriangleRight, UmbrellaIcon, WatchIcon, Zap } from 'lucide-react'
...
const brandsWithIcon = [
  ...
  { id: "adidas", label: "Adidas", icon: TriangleRight },
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="relative w-full h-[600px] overflow-hidden">
        {
          featureImageList && featureImageList.length > 0 ? featureImageList.map((slide, index) => <img src={slide.image} key={index} className={`${index === currentSlide ? "opacity-100" : "opacity-0"} absolute top-0 left-0 w-full h-full object-cover transition-opacity duration-1000`} />) : null
        }
      ...
      <section className='py-12 bg-gray-50'>
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {
              brandsWithIcon.map(brandItem => <Card key={brandItem.id} onClick={() => handleNavigateToListingPage(brandItem, "brand")} className="cursor-pointer hover:shadow-lg transition-shadow">
                <CardContent className="flex flex-col items-center justify-center p-6">
